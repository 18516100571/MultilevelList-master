...
import com.demo.multilevellist.test.TestTreePoint;
...
public class TreeUtils {
    ...
    //第一级别为0
    public static int getLevel(TestTreePoint treePoint, HashMap<String,TestTreePoint> map){
        if("0".equals(treePoint.getPARENTID())){
            return 0;
        }else{
            return 1+getLevel(getTreePoint(treePoint.getPARENTID(),map),map);
    ...
    public static TestTreePoint getTreePoint(String ID, HashMap<String,TestTreePoint> map){
        if(map.containsKey(ID)){
            return map.get(ID);
        }
        Log.e("xlc","ID:" + ID);
